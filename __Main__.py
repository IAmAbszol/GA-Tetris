...
class Main:
    ...
    # @param parameters - Select of n genes that help evaluate the games positioning for a tetris shape object
    def gameLoop(self, parameters):
        # initiate game - keeping to the params of creators design
        self.doRestart()
        while True:
            if self.g.gameOver:
                break
    ...
    # implement main for genetic algorithm
    def get_fitness(self, genes):
        return self.gameLoop(genes)

    def display(self, candidate, startTime):
        timeDiff = datetime.datetime.now() - startTime
        print("Selected Parent Scored {} - {}\t{}.".format(candidate.Fitness, candidate.Genes, timeDiff))

    def solveTetris(self):
        geneRange = [ i for i in range(-10, 10) ]
        ...
        optimalFitness = 1000000
        best = genetic.get_best(fnGetFitness, 4, optimalFitness, geneset, fnDisplay, custom_mutate=None)
